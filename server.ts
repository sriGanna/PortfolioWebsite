        ...
        try {
            ...
            await fetch(ttsResponse["audio"]).then(resp => {
                ...
                if (hasAudioHaptic && segArray.length > 0 
                    ...
                    //         size: 1 };    
                    // } 
                    // for (let i = 1 + hapticSegInfo.length + 1; i < segArray.length; i++) {
                    //     s[i] = {...s[i], centroid: [hapticObjInfo?.[i - 1]?.['centroid']],
                    //     contourPoints: [hapticObjInfo?.[i - 1]?.['coordinates']],
                    //     size: 1};
                    // }
